...
export const Route = createFileRoute("/_authenticated/friends")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "أصدقاء التاج وطلبات الصداقة الواردة والمرسلة بتحديث مباشر.",
      },
...
function FriendsPage() {
  ...
  const relations = useQuery({
    ...
    queryFn: async () => {
      ...
      const ids = [
        ...new Set(rows.map((r) => (r.requester_id === userId ? r.partner_id : r.requester_id))),
      ];
      let people: Profile[] = [];
      if (ids.length > 0) {
        const result = await supabase
  ...
  const profileById = useMemo(
    () => new Map((data.data?.profiles ?? []).map((p) => [p.id, p])),
    [data.data?.profiles],
  );
  const incoming = (data.data?.requests ?? []).filter(
    (r) => r.addressee_id === userId && r.status === "pending",
  );
  const sent = (data.data?.requests ?? []).filter(
    (r) => r.requester_id === userId && r.status === "pending",
  );

  return (
    <AppShell
      header={<PageHeader title="الأصدقاء" subtitle={`${data.data?.friends.length ?? 0} صديق`} />}
    >
      ...
      {data.isLoading ? (
        ...
        (data.data?.friends.length ?? 0) === 0 ? (
          <EmptyState title="لا يوجد أصدقاء بعد" hint="أرسل طلبًا من الملف الشخصي لأي مستخدم" />
        ) : (
          <div className="space-y-2">
            {data.data?.friends.map((row) => {
              ...
              return (
                <PersonRow key={row.friend_id} person={person}>
                  <Link
                    to="/messages/$userId"
                ...
                <div key={row.id} className="rounded-2xl">
                  <PersonRow person={person}>
                    {incomingRelation ? (
                      <>
                        <Button
                          onClick={() => relationAction.mutate({ id: row.id, action: "accept" })}
                          className="h-9 w-9 rounded-xl p-0"
                          aria-label="قبول العلاقة"
                        >
                          <Check className="h-4 w-4" />
                        </Button>
                        <Button
                          variant="outline"
                          onClick={() => relationAction.mutate({ id: row.id, action: "reject" })}
                          className="h-9 w-9 rounded-xl p-0"
                          aria-label="رفض العلاقة"
                        >
                          <X className="h-4 w-4" />
                        </Button>
                      </>
                      ...
                      <Button
                        variant="outline"
                        onClick={() => relationAction.mutate({ id: row.id, action: "end" })}
        ...
        incoming.length === 0 ? (
          <EmptyState title="لا توجد طلبات واردة" />
        ) : (
          <div className="space-y-2">
            {incoming.map((request) => {
              ...
              if (!person) return null;
              return (
                <PersonRow key={request.id} person={person}>
                  <Button
        ...
        <EmptyState title="لا توجد طلبات مرسلة" />
      ) : (
        <div className="space-y-2">
          {sent.map((request) => {
