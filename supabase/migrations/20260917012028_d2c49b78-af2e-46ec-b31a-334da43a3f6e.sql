CREATE OR REPLACE FUNCTION app_internal.send_room_message_impl(_uid uuid, _room_id uuid, _body text)
...
BEGIN
  ...
  IF locked AND NOT app_internal.can_manage_room(_room_id, _uid) THEN
    RAISE EXCEPTION 'الدردشة مغلقة';
  END IF;

  INSERT INTO public.room_messages (room_id, user_id, body, kind)
  VALUES (_room_id, _uid, btrim(_body), 'text')
  RETURNING * INTO result;
  RETURN result;
END;
$$;

REVOKE ALL ON FUNCTION app_internal.send_room_message_impl(uuid, uuid, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION app_internal.send_room_message_impl(uuid, uuid, text) TO service_role;
...
SET search_path = public, app_internal
AS $$
  SELECT app_internal.send_room_message_impl(auth.uid(), _room_id, _body)
$$;
